...
with open("../etc/shakespeare.txt", "r", encoding='utf-8') as f:
	text = f.read()
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb # (B, T, C) + (T, C) ----> (B, T, C) [broadcasting semantics]
        x = self.blocks(x)
        x = self.ln_f(x)
        # x = self.sa_heads(x) # apple one head of self-attention
        # x = self.ffwd(x)
...
# this code will literally BURN my macbook
# open('../etc/more.txt', 'w').write(decode(m.generate(context, max_new_tokens=10000)[0].tolist()))